document.addEventListener('DOMContentLoaded', function() {
  ...
  const searchDescripcion = document.getElementById('searchDescripcion');
  ...
  // Filtrar productos por categoría y descripción
  function filterProductos() {
    const categoriaFilter = searchCategoria.value.toLowerCase();
    const descripcionFilter = searchDescripcion.value.toLowerCase();
    
    const filteredProductos = productos.filter(prod => {
      const matchesCategoria = prod.categoria.toLowerCase().includes(categoriaFilter);
      const matchesDescripcion = prod.descripcion.toLowerCase().includes(descripcionFilter);
      return matchesCategoria && matchesDescripcion;
    });

    // Renderizar la tabla con los productos filtrados
    renderTableWithData(filteredProductos);
  }
  ...
  // Escuchar eventos en los campos de búsqueda
  searchCategoria.addEventListener('input', filterProductos);
  searchDescripcion.addEventListener('input', filterProductos);
